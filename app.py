...
<div class="hero-banner">
  ...
  <div class="hero-badge">v4.2 · Slab-Based Fixed Fee (Brand × Category × Selling Price)</div>
</div>
""", unsafe_allow_html=True)
...
ORDER_TO_PRICE_SIZE = defaultdict(list)
...
with st.sidebar:
    ...
    closed_sku_file = st.file_uploader(
        ...
        type=["xlsx"],
        help="Column 1: SKU  |  Column 2: Closed Sku Price"
    )
    st.markdown("---")
    st.markdown("### ⚙️ Settings")
    c1, c2 = st.columns(2)
    fixed_fee_fallback = c1.number_input(
        "Fixed Fee Fallback (₹)", value=5, min_value=0, step=1,
        help="Used only when no Fixed Fee slab matches in Sheet 0 for a given Brand + Category + Selling Price"
    )
...
<b>Commission %</b> → slab by Invoice Amount, rate on Selling Price<br>
<b>Collection %</b> → slab by Invoice Amount, rate on Selling Price<br>
<b>Fixed Fee ₹</b> → slab by <b>Selling Price</b> (cols: Lower/Upper Limit Fixed Fee)<br>
&nbsp;&nbsp;Fallback: sidebar value if no slab found<br>
...
def _extract_numeric_code(sku):
    m = re.search(r"(\d{3,})", str(sku))
    return m.group(1) if m else ""

def _valid_sub_cat(ci):
    sc = ci.get("sub_cat", "")
    return bool(sc) and str(sc).strip().lower() not in ("", "nan")

def lookup_sub_cat(raw_sku, sku_info_dict):
    ...
    for csk, ci in sku_info_dict.items():
        if not _valid_sub_cat(ci): continue
        sc = ci["sub_cat"]
        if csk == key_raw: return sc, "exact"
        if csk == stripped: return sc, "exact-stripped"
        if _norm_sku(csk) == norm_raw or _norm_sku(csk) == norm_str: return sc, "exact-norm"
        ...
        elif csk_base_norm in (base_norm, get_sku_base(norm_str)): t6_hits.append((csk, sc))
        if num_code and len(num_code) >= 3 and num_code in _norm_sku(csk): t7_hits.append((csk, sc))
        if len(key_raw) >= 6 and csk.startswith(key_raw[:6]): t8_hits.append((csk, sc))
...
def lookup_pwn(sku, pwn_dict):
    ...
    for csk, cv in pwn_dict.items():
        if _norm_sku(csk) == norm_key and pd.notna(cv): return float(cv), f"norm-exact({csk})"
    norm_base = get_sku_base(norm_key)
    if norm_base:
        for csk, cv in pwn_dict.items():
            if get_sku_base(_norm_sku(csk)) == norm_base and pd.notna(cv): return float(cv), f"norm-base({csk})"
    num_code = _extract_numeric_code(key)
    if num_code and len(num_code) >= 4:
        for csk, cv in pwn_dict.items():
            if num_code in _norm_sku(csk) and pd.notna(cv): return float(cv), f"num-code({csk})"
...
def lookup_closed_sku(raw_sku, closed_sku_dict):
    if not closed_sku_dict: return np.nan, False
    key_raw  = raw_sku.strip().upper()
    stripped = strip_vendor_prefix(raw_sku).strip().upper()
    norm_raw = _norm_sku(key_raw)
    norm_str = _norm_sku(stripped)
    v = closed_sku_dict.get(key_raw)
    if v is not None and pd.notna(v): return float(v), True
    v = closed_sku_dict.get(stripped)
    if v is not None and pd.notna(v): return float(v), True
    for csk, cv in closed_sku_dict.items():
        if _norm_sku(csk) in (norm_raw, norm_str) and pd.notna(cv): return float(cv), True
    base_raw = get_sku_base(key_raw)
    base_str = get_sku_base(stripped)
    for csk, cv in closed_sku_dict.items():
        if get_sku_base(csk) in (base_raw, base_str) and pd.notna(cv): return float(cv), True
    return np.nan, False
...
def _filter_brand_cat(charges_df, brand, cat):
    ...
    if not exact.empty: return exact.drop(columns=["_cat_norm"])
    sub = brand_rows[brand_rows["_cat_norm"].apply(lambda x: x in cat_norm or cat_norm in x)]
    if not sub.empty: return sub.drop(columns=["_cat_norm"])
    cat_nospace = cat_norm.replace(" ", "")
    sub2 = brand_rows[brand_rows["_cat_norm"].apply(lambda x: x.replace(" ","") == cat_nospace)]
    if not sub2.empty: return sub2.drop(columns=["_cat_norm"])
    ...
    if not tok.empty: return tok.drop(columns=["_cat_norm","_overlap"])
    return pd.DataFrame()
...
def lookup_gt(brand, cat, inv_amount, charges_df):
    for _, r in _filter_brand_cat(charges_df, brand, cat).iterrows():
        ...
        try:
            if float(lo) <= inv_amount <= float(hi): return float(gt)
        except: continue
    return np.nan
...
def lookup_fixed_fee(brand, cat, sell_price, charges_df, fallback):
    ...
    Returns (fee_value, method_string)
    """
    for _, r in _filter_brand_cat(charges_df, brand, cat).iterrows():
        lo  = r.get("Lower Limit Fixed Fee")
        hi  = r.get("Upper Limit Fixed Fee")
        fee = r.get("Fixed Fee")
...
def _parse_charges_df(raw):
    ...
    numeric_cols = [
        ...
        # NEW Fixed Fee slab columns
        "Lower Limit Fixed Fee","Upper Limit Fixed Fee","Fixed Fee",
    ]
    ...
    if "Collection Charge" in df.columns:
        df["Collection Charge"] = (df["Collection Charge"].astype(str)
            .str.replace("Rs","",regex=False).str.replace("₹","",regex=False).str.strip()
...
def parse_closed_sku_dict(file_bytes):
    raw = pd.read_excel(BytesIO(file_bytes), sheet_name=0, header=None)
    df = raw.copy()
    df.columns = [str(c).strip() for c in raw.iloc[0].tolist()]
    df = df.iloc[1:].reset_index(drop=True)
    cols = df.columns.tolist()
    sku_col, price_col = cols[0], cols[1]
...
def parse_replace_map_cached(file_bytes):
...
def run_reconciliation(order_df, charges_df, sku_info_dict, pwn_dict,
    ...
    for _, row in order_df.iterrows():
        ...
        if pd.isna(gt_val):
            ...
            fixed_fee_val    = float(fixed_fee_fallback)
            fixed_fee_method = "fallback"
            if not brand_name:        charge_method = "no_brand_in_product"
            elif not cat:             charge_method = f"no_subcat|brand={brand_name}"
            else:                     charge_method = f"slab_missing|{brand_name}|{cat}"
        else:
            commission    = commission if pd.notna(commission) else 0.0
            coll_fee      = coll_fee   if pd.notna(coll_fee)   else 0.0
            total_charges    = round(commission + coll_fee + fixed_fee_val, 5)
        ...
        # ── Step 8: PWN lookup ─────────────────────────────────────────────
        pwn_val = np.nan; match_method = "not_found"; is_closed_sku = False
        ...
        if sku_for_override in pwn_overrides:
            ...
            if found_closed:
                pwn_val = closed_price; match_method = "closed_sku"; is_closed_sku = True
            else:
        ...
        rows_out.append({
            ...
            "Fixed Fee":         fixed_fee_val,       # slab-resolved Rs amount (or fallback)
            "Fixed Fee Method":  fixed_fee_method,    # "slab(lo-hi)" or "fallback"
...
def apply_roc_sheet_style(ws, df):
    ...
    col_widths = {
        ...
        "Commission":14,"Collection Fee":15,
        "Fixed Fee":12,"Fixed Fee Method":20,
        "Total Charges":15,"GST on Charges":15,"Taxable Value":14,
        "TDS":10,"TCS":10,"Total Deductions":16,"Received Amount":16,
        "PWN":12,"PWN Benchmark":15,"PWN Match":16,
        "Closed SKU":10,"Difference":14,"Source File":20,
    }
    ...
    def row_formulas(r):
        ...
        pwn=C.get("PWN",""); pb=C.get("PWN Benchmark",""); diff=C.get("Difference","")
        fmls = {}
...
def build_summary(df):
    ...
    ff_fallback_count = int((df.get("Fixed Fee Method", pd.Series([])) == "fallback").sum()) if "Fixed Fee Method" in df.columns else 0
    ff_slab_count = len(valid) - ff_fallback_count
    for label, val in [
        ("Total Orders",              len(df)),
        ...
        ("Fixed Fee via Slab",        ff_slab_count),
        ("Fixed Fee via Fallback",    ff_fallback_count),
    ...
    cat_df = (
        ...
        .agg(Orders=("Order Id","count"),Invoice_Total=("Invoice Amount","sum"),
             GT_Total=("GT (As Per Calc)","sum"),Selling_Total=("Selling Price","sum"),
             Commission=("Commission","sum"),Collection=("Collection Fee","sum"),
             Fixed=("Fixed Fee","sum"),Total_Charges=("Total Charges","sum"),
             GST_Total=("GST on Charges","sum"),TDS_Total=("TDS","sum"),TCS_Total=("TCS","sum"),
             Deductions=("Total Deductions","sum"),Received_Total=("Received Amount","sum"),
             Net_Diff=("Difference","sum"),Avg_Diff=("Difference","mean"))
    ...
    brand_df = (
        ...
        .agg(Orders=("Order Id","count"),Invoice_Total=("Invoice Amount","sum"),
             Selling_Total=("Selling Price","sum"),Total_Charges=("Total Charges","sum"),
             Deductions=("Total Deductions","sum"),Received_Total=("Received Amount","sum"),
             Net_Diff=("Difference","sum"))
        .reset_index().sort_values("Invoice_Total", ascending=False).round(2)
    )
    brand_df.columns = ["Brand","Orders","Invoice Total","Selling Total",
    ...
    if "Closed SKU" in df.columns:
        ...
        if not closed_rows.empty:
            ...
            closed_df = pd.DataFrame({
                "Metric": ["Total Closed SKU Orders","Total Invoice Amount",
                           "Total Received Amount","Net Difference vs Closed PWN",
                           "Orders Negative Diff","Orders Positive Diff"],
                "Value":  [len(closed_rows),
...
DISPLAY_COLS = [
    ...
    "Selling Price","Commission","Collection Fee",
    "Fixed Fee","Fixed Fee Method",
    "Total Charges","GST on Charges","Taxable Value","TDS","TCS","Total Deductions",
...
if order_files and charges_file:
    ...
    with st.spinner("Reading and parsing files…"):
        ...
        # Check Fixed Fee slab columns
        ff_cols_present = {"Lower Limit Fixed Fee","Upper Limit Fixed Fee","Fixed Fee"}.issubset(set(charges_df.columns))
        if ff_cols_present:
            ff_slab_rows = charges_df[["Lower Limit Fixed Fee","Upper Limit Fixed Fee","Fixed Fee"]].dropna().shape[0]
            st.sidebar.success(f"✅ Fixed Fee slab columns found — **{ff_slab_rows}** slab rows detected")
        else:
            missing_ff = {"Lower Limit Fixed Fee","Upper Limit Fixed Fee","Fixed Fee"} - set(charges_df.columns)
            st.sidebar.warning(
                f"⚠️ Fixed Fee slab column(s) missing in Sheet 0: {', '.join(missing_ff)}. "
                f"All rows will use fallback ₹{fixed_fee_fallback}."
            )
    ...
    with st.expander("🔍 Brand & Sub-Category Detection — first 20 rows", expanded=False):
        ...
        if closed_sku_dict:
            sample["Closed SKU?"] = sample["SKU"].apply(lambda s: "✅" if lookup_closed_sku(s, closed_sku_dict)[1] else "")
        ...
        col_b.markdown("**Categories (raw → normalised):**\n\n" + "\n\n".join(cats_norm))
    ...
    if closed_sku_dict:
        with st.expander(f"🔒 Closed SKU Preview — {len(closed_sku_dict):,} entries", expanded=False):
            st.dataframe(pd.DataFrame([{"SKU":k,"Closed Price (₹)":v} for k,v in list(closed_sku_dict.items())[:30]]),
    ...
    ff_slab_count = int(result_df.get("Fixed Fee Method", pd.Series([])).str.startswith("slab", na=False).sum()) if "Fixed Fee Method" in result_df.columns else 0
    ff_fall_count = int((result_df.get("Fixed Fee Method", pd.Series([])) == "fallback").sum()) if "Fixed Fee Method" in result_df.columns else 0
    replace_resolved = result_df[result_df["PWN Match"].str.startswith("replace", na=False)]

    st.success(
        f"✅ **{len(result_df):,}** orders processed  |  **{calc_count:,}** calculated  |  **{skip_count:,}** skipped"
        + (f"  |  🔒 **{closed_count:,}** via Closed SKU" if closed_count else "")
        + (f"  |  📐 Fixed Fee: **{ff_slab_count:,}** via slab / **{ff_fall_count:,}** via fallback" if ff_slab_count or ff_fall_count else "")
    ...
    if closed_count: tabs_list.append("🔒  Closed SKU")
    ...
    with tab1:
        ...
        if len(broken_df):
            ...
            with st.expander(f"✏️  **{len(broken_df)} SKU(s) have lookup issues** — click to fix", expanded=False):
        ...
        if st.session_state["sku_corrections"]:
            with st.expander(f"✅  **{len(st.session_state['sku_corrections'])} correction(s) active**", expanded=False):
                st.dataframe(pd.DataFrame([{"Original SKU":o,"→ Corrected SKU":c} for o,c in st.session_state["sku_corrections"].items()]),
                             use_container_width=True, hide_index=True)
        ...
        nd_col, closed_col, ff_col, _ = st.columns([1,1,1,2])
        nd_col.metric("Net Difference vs PWN", f"₹{net:,.2f}",
                      delta=f"{'▲' if net>=0 else '▼'} {abs(net):,.2f}",
                      delta_color="normal" if net>=0 else "inverse")
        if closed_count: closed_col.metric("🔒 Closed SKU Orders", f"{closed_count:,}")
        if "Fixed Fee Method" in result_df.columns:
            ff_col.metric("📐 Fixed Fee Slab / Fallback", f"{ff_slab_count:,} / {ff_fall_count:,}",
                          help="Slab = matched row in Sheet 0 | Fallback = sidebar default")
        st.markdown("---")
        ...
        diff_opt  = f3.selectbox("Difference type", [
            ...
            "No Category (NaN)","Closed SKU Only","Fixed Fee Fallback Only"
        ])
        ...
        if   diff_opt == "Positive (+)":          view = view[view["Difference"] > 0]
            ...
            if "Closed SKU" in view.columns: view = view[view["Closed SKU"] == "✅"]
        elif diff_opt == "Fixed Fee Fallback Only":
            if "Fixed Fee Method" in view.columns: view = view[view["Fixed Fee Method"] == "fallback"]
        if search.strip():
        ...
        st.dataframe(style_table(view[avail_cols], diff_col="Difference"), use_container_width=True, height=500)
        ...
        d1.download_button("⬇  Full Reconciliation (Excel · styled)",
    ...
    with tab2:
        ...
        st.info(
            "ℹ️  **GT** → slab on Invoice Amount  |  "
            "**Commission & Collection %** → slab by Invoice Amount, applied on Selling Price  |  "
            "**Fixed Fee ₹** → slab by **Selling Price** (Lower/Upper Limit Fixed Fee in Sheet 0); "
            "fallback = sidebar value if no slab matches  |  "
            "**Received** = SP − Charges − GST − TDS − TCS  |  "
        ...
        charge_cols = ["Order Id","SKU","Brand Name","Sub-Category","Charge Method",
                       "Invoice Amount","GT (As Per Calc)","Selling Price","Commission",
                       "Collection Fee","Fixed Fee","Fixed Fee Method","Total Charges",
                       "GST on Charges","Taxable Value","TDS","TCS","Total Deductions","Received Amount"]
    ...
    with tab3:
        ...
        st.dataframe(style_table(cat_df, diff_col="Net Difference").format({c:"₹{:.2f}" for c in cat_money}), use_container_width=True)
        st.markdown("---")
        st.markdown("#### 🔢 Charge Components (per Sub-Category)")
        comp_cols = ["Sub-Category","Orders","GT Total","Commission","Collection Fee","Fixed Fee","GST Total","TDS Total","TCS Total","Total Deductions"]
        comp_money = [c for c in comp_cols if c not in ("Sub-Category","Orders")]
        avail_comp = [c for c in comp_cols if c in cat_df.columns]
        st.dataframe(cat_df[avail_comp].style.format({c:"₹{:.2f}" for c in comp_money}), use_container_width=True)
    ...
    with tab4:
        ...
        st.dataframe(style_table(brand_df, diff_col="Net Difference").format({c:"₹{:.2f}" for c in brand_money}), use_container_width=True)
        st.markdown("---")
        st.markdown("#### 📋 Brand × Sub-Category Detail")
        valid2 = result_df[result_df["Received Amount"].notna()]
        bxc = (valid2.groupby(["Brand Name","Sub-Category"])
               .agg(Orders=("Order Id","count"),Invoice=("Invoice Amount","sum"),
                    Received=("Received Amount","sum"),Net_Diff=("Difference","sum"))
               .reset_index().round(2))
        bxc.columns = ["Brand","Sub-Category","Orders","Invoice Total","Received Total","Net Difference"]
        st.dataframe(bxc.style.format({c:"₹{:.2f}" for c in ["Invoice Total","Received Total","Net Difference"]}),
    ...
    if tab_closed is not None:
        with tab_closed:
            ...
            st.info(f"**{closed_count:,}** orders matched a Closed SKU. Their PWN benchmark uses the Closed SKU price file.")
            ...
            if not closed_rows.empty:
                ...
                st.dataframe(style_table(closed_rows[avail_closed], diff_col="Difference"), use_container_width=True, height=500)
                st.download_button("⬇  Download Closed SKU Orders (CSV)",
                    data=closed_rows[avail_closed].to_csv(index=False).encode(),
                    file_name="flipkart_closed_sku_orders.csv", mime="text/csv")
